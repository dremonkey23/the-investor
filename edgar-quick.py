"""Quick EDGAR scraper - resilient version."""
...
# Form 4
url = f"https://efts.sec.gov/LATEST/search-index?forms=4&dateRange=custom&startdt={start}&enddt={today}&from=0&size=15"
with httpx.Client(timeout=8, headers=HEADERS) as c:
    r = c.get(url)
    hits = r.json().get("hits", {}).get("hits", []) if r.status_code == 200 else []
    print(f"EFTS Form 4: {len(hits)} hits")
    ...
    for hit in hits:
        ...
        try:
            idx = c.get(filing_url, timeout=5)
            if idx.status_code == 200:
                ...
                if target:
                    time.sleep(0.3)
                    xr = c.get(f"{filing_url}{target}", timeout=5)
                    if xr.status_code == 200:
                        try:
                            ...
                            notable = txn_type == "Purchase" and total_val > 100000
                        except Exception:
                            pass
        except Exception:
            pass
...
print(f"\nSaved: {len(trades)} trades, {len(events)} events")
if notable_list:
    print(f"NOTABLE BUYS: {len(notable_list)}")
    for t in notable_list:
        print(f"  {t['insiderName']} bought ${t['totalValue']:,.0f} of {t['ticker']}")
print("Done!")
